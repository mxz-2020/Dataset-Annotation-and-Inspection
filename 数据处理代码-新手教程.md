...
# 读入hdf5格式文件
f = h5py.File(filename, 'r') ###filename为对应的文件路径

# 查看根下的所有group
list(f.keys())  ##类似一个字典，一般为mm10，hg38等
# Output eg: ['mm10']
...
# 查看 mm10的子group
list(groups.keys())
# Output eg: ['barcodes', 'data', 'gene_names', 'genes', 'indices', 'indptr', 'shape']
...
# Column				Type				Description
# barcodes			string				 cellID
# data					  uint32			矩阵不为0的值
# indices				uint32			  矩阵中为0的坐标（行）
# indptr				uint32			   矩阵中为0的坐标（列）
# shape					uint64			 行数和列数

# 获取gene名
genes= list(groups['gene_names'])###一般建议提取groups['genes']，即ensemblID作为gene；
# 转为utf-8格式
genes = [str(g, encoding='utf-8') for g in genes] ##强制转换格式，避免出错

# 获取cellID
barcodes =list(groups['barcodes'])
# 转为utf-8格式
barcodes = [str(b, encoding='utf-8') for b in barcodes]

# 生成矩阵
data = groups['data']
indices = groups['indices']
indptr = groups['indptr']
shape = groups['shape']
matrix = sparse.csc_matrix((data, indices, indptr), shape=shape)


# 根据 matrix的shape来判断是否需要矩阵转置
# matrix = matrix.T
# 转为coo矩阵
matrix = matrix.tocoo()
##最后用模板里的代码保存稀疏矩阵即可
my_builder.save_template_mtx(mtx=matrix , mtx_name='expressionMatrix_rawCounts.mtx', genes=genes, cellID=barcodes)

# 参考链接
# 1. https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/advanced/h5_matrices
# 2. https://segmentfault.com/a/1190000016670881
```
