...
- [ 二、cellAnnotion处理](#head11)
        - [例子1: cellID刚好能够和title对应](#head12)
        - [例子2: 存在某个线索可以实现cellID和sampleID的转化](#head13)
- [ 三、快速填写marker基因](#head14)
	- [情况1： markerGene文件以一个表格的形式存放](#head15)
	- [情况2： markerGene文件以多重Excel表格的形式存放,不同的sheet用对应的clusterName表示](#head16)
...
##### <span id="head12">例子1: cellID刚好能够和title对应</span>
...
##### <span id="head13">例子2: 存在某个线索可以实现cellID和sampleID的转化</span>
...
```python
df_cell['sampleID'] = df_cell['cellID'].str.replace('([ATCG]+-1)','GSM4231663') ##这段代码是正则匹配替换，意思是：将cellID中任意多个A/T/C/G四种字符的组合加-1的字符替换成GSM4231663;并且保存到df_cell['sampleID'] 中；后几句同理；
...
# <span id="head14"> 三、快速填写marker基因</span>
### <span id="head15">情况1： markerGene文件以一个表格的形式存放</span>
如下图：
```python
...
```
### <span id="head16">情况2： markerGene文件以多重Excel表格的形式存放,不同的sheet用对应的clusterName表示</span>
如下图：
